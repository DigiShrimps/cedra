// Code taken from https://github.com/mofalabs/aptos/tree/main/lib/bcs

import 'dart:convert';
import 'dart:typed_data';
import 'package:cedra_basic_sdk/src/bcs/consts.dart';
...
extension SerializerAddress on Serializer {
  void serializeAddress(String hex) {
    final cleaned = hex.startsWith('0x') ? hex.substring(2) : hex;
    final bytes = Uint8List.fromList(List<int>.generate(32, (i) {
      final byteIndex = i * 2;
      if (byteIndex >= cleaned.length) return 0;
      final byte = cleaned.substring(byteIndex, byteIndex + 2);
      return int.parse(byte, radix: 16);
    }));
    serializeFixedBytes(bytes);
  }
}